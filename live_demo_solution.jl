...
begin
	kLL_c(x, x′) = kLL(x, x′) - 2c_r * κ_1d * d2k(x, x′) + c_r^2 * k_se(x, x′)   #SOL
	kLu_c(x, x′) = kLu(x, x′) + c_r * k_se(x, x′)   #SOL
	kuL_c(x, x′) = kuL(x, x′) + c_r * k_se(x, x′)   #SOL
end
...
begin
	...
	ℒ_lap = δ(X_c_2d) ∘ (PartialDerivative((2, 0)) + PartialDerivative((0, 2)))   #SOL
end
...
my_sensors = [(0.3, 1.0), (0.7, 1.0)]   #SOL
...
begin
	...
	K_Lu = [kLu(x, x′) for x in X_pde, x′ in X_bc]   #SOL
	K_uL = [kuL(x, x′) for x in X_bc, x′ in X_pde]   #SOL
...
begin
	...
	K_cross = hcat([kuL(x, x′) for x in X_pred, x′ in X_pde], [k_se(x, x′) for x in X_pred, x′ in X_bc])   #SOL
...
@latte function harbor_source_model(y_phys, y_bc, y_sensor, X_c, field)
	...
	for i in eachindex(X_c)
		...
		r_i = blocks.adv[i] - κ_TRUE * blocks.Δu[i] + blocks.u[i] - f_i   #SOL
